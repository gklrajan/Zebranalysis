...
window_size         = 60;
num_data_categories = 6+window_size^2;
camscale_px_per_mm  = 20.6; % px/mm
datarate_Hz         = 949;  % Hertz
NumVar              = 6;
...
% SYSTEM TIME: 
% system time difference between frames in ms as a measure of system load

system_time_diff  = [0; diff(tmp_data(:, 3))];  % in ms 
...
%%
% read the fish images from the data 

% Read the 3600px associated with each frame and store it column-wise

tmp_movie_data = tmp_data(:, NumVar+1:end)';

% reshape into a [60, 60, frames] matrix

CXPimg  = reshape(tmp_movie_data, [ 60, 60, size(tmp_movie_data, 2) ] );

figure; imshow3D(CXPimg);
...
filter    = ones(1, 23)./23; % filter tbd
...
tmp_ori       = diff(tmp_data(:, 6));
tmp_delta_ori = [0; tmp_ori];
